...
export default function EditSeriesForm() {
  ...
  const { data, loading, error } = GetSeriesById(seriesId);
  console.log(data, loading, error, "edittttseries");
  ...
  const {
    ...
    onSubmit: async (values) => {
      try {
        ...
        const response = await updateSeriesByIdurl(seriesId, formData);
      ...
      <Card className="w-full">
        <CardHeader>
          <div className="space-y-1.5">
            <h2 className="text-2xl font-medium leading-6">Edit Series</h2>
